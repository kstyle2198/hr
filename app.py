...
def 사무설계연구시뮬(df, 회사, 시작기준시점, 퇴사시점, 채용시점, 승급시점, random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율):
    ...
    retire_df = 사무설계연구퇴직(start_df, 퇴사시점, 직급별퇴사율, random_state)
    print(f"-------------{회사} retire_df: {retire_df.shape}---------------------")

    recruit_df = 사무설계연구채용(retire_df, 회사, 채용시점, random_state, 채용인원비율들, 채용대상직급, 남녀선택)
    print(f"-------------{회사} recruit_df: {recruit_df.shape}---------------------")

    promotion_df = 사무설계연구승급(recruit_df, 승급시점, 직급별승진율)
...
def story_of_future():
    ...
    with st.form("sim-conditions"):
        ...
        with co1501:
            사원퇴사율 = st.selectbox("HL1 퇴사율", [0.05, 0.10, 0.08, 0.04, 0.03])
        with col502:
            대리퇴사율 = st.selectbox("HL2 퇴사율", [0.05, 0.10, 0.08, 0.04, 0.03])
        with col503:
            과장퇴사율 = st.selectbox("HL3(1) 퇴사율", [0.04, 0.08, 0.04, 0.03])
        with col504:
            차장퇴사율 = st.selectbox("HL3(2) 퇴사율", [0.03, 0.04, 0.02, 0.01])
        with col505:
            부장퇴사율 = st.selectbox("HL3(3) 퇴사율", [0.01, 0.03])   
        직급별퇴사율 = [사원퇴사율, 대리퇴사율, 과장퇴사율, 차장퇴사율, 부장퇴사율]

        st.markdown("**📌 직급별 채용율**")
        co1601, col602, col603, col604 = st.columns([1, 1, 1, 1])
        with co1601:
            사원채용율 = st.selectbox("HL1 채용율", [0.3, 0.2, 0.1, 0.05])
        with col602:
            대리채용율 = st.selectbox("HL2 채용율", [0.3, 0.2, 0.1, 0.05])
        with col603:
            과장채용율 = st.selectbox("HL3(1) 채용율", [0.2, 0.1, 0.08, 0.05])
        with col604:
            차장채용율 = st.selectbox("HL3(2) 채용율", [0.1, 0.08, 0.05, 0.03])
        채용인원비율들 = [사원채용율, 대리채용율, 과장채용율, 차장채용율]
        
        st.markdown("**🐬 직급별 승진율**")
        co1601, col602, col603, col604 = st.columns([1, 1, 1, 1])
        with co1601:
            대리승진율 = st.selectbox("HL2 승진율", [0.7, 0.5, 0.3])
        with col602:
            과장승진율 = st.selectbox("HL3(1) 승진율", [0.4, 0.3, 0.1])
        with col603:
            차장승진율 = st.selectbox("HL3(2) 승진율", [0.2, 0.1, 0.08, 0.05])
        with col604:
            부장승진율 = st.selectbox("HL3(3) 승진율", [0.1, 0.08, 0.05, 0.03])
        직급별승진율 = [대리승진율, 과장승진율, 차장승진율, 부장승진율]        
        
        st.markdown("---")

        # 2023년 시뮬레이션
        HG_df1 = 사무설계연구시뮬(df, select201, "t20230101", "t20230401", "t20230801", "t20240101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HDI_df1 = 사무설계연구시뮬(df, "HDI", "t20230101", "t20230401", "t20230801", "t20240101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HCE_df1 = 사무설계연구시뮬(df, "HCE", "t20230101", "t20230401", "t20230801", "t20240101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)

        # 2024년 시뮬레이션
        HG_df1 = 사무설계연구시뮬(HG_df1, select201, "t20240101", "t20240401", "t20240801", "t20250101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HDI_df1 = 사무설계연구시뮬(HDI_df1, "HDI", "t20240101", "t20240401", "t20240801", "t20250101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HCE_df1 = 사무설계연구시뮬(HCE_df1, "HCE", "t20240101", "t20240401", "t20240801", "t20250101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        
        # 2025년 시뮬레이션
        HG_df1 = 사무설계연구시뮬(HG_df1, select201, "t20250101", "t20250401", "t20250801", "t20260101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HDI_df1 = 사무설계연구시뮬(HDI_df1, "HDI", "t20250101", "t20250401", "t20250801", "t20260101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        # HCE_df1 = 사무설계연구시뮬(HCE_df1, "HCE", "t20250101", "t20250401", "t20250801", "t20260101", random_state, 직급별퇴사율, 채용인원비율들, 직급별승진율)
        ...
        if submitted:
            ...
            print(total_df.info())
            print(total_df.기준일자.value_counts().sort_index())
